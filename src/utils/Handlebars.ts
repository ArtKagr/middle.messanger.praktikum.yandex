...
type ComponentType<T extends BlockComponentClass<T>> = { // @ts-ignore
    new (props: ConstructorParameters<InstanceType>)
}
...
export function registerComponent<T extends BlockComponentClass<T>>(Component: ComponentType<T>) {
    ...
    Handlebars.registerHelper(Component.componentName, function (this: unknown, { hash, data, fn }: HelperOption) => {
